...
namespace cheat::feature
{
	...
	FreeCamera::FreeCamera() : Feature(),
		...
		NF(f_DamageOverlay, u8"损伤覆盖", "Visuals::FreeCamera", false),
		NF(f_HpOverlay, u8"敌人HP覆盖", "Visuals::FreeCamera", false),
		NF(f_Speed, u8"速度", "Visuals::FreeCamera", 1.0f),
		NF(f_LookSens, u8"Look Sensitivity", "Visuals::FreeCamera", 1.0f),
		...
		NF(f_MovSmoothing, u8"平滑运动", "Visuals::FreeCamera", 1.0f),
		NF(f_LookSmoothing, u8"Look Smoothing", "Visuals::FreeCamera", 1.0f),
		NF(f_RollSmoothing, u8"滚平", "Visuals::FreeCamera", 1.0f),
		NF(f_FovSmoothing, u8"视场平滑", "Visuals::FreeCamera", 1.0f),
		NF(f_Forward, u8"向前地", "Visuals::FreeCamera", Hotkey('W')),
		NF(f_Backward, u8"向后地", "Visuals::FreeCamera", Hotkey('S')),
		NF(f_Left, u8"左", "Visuals::FreeCamera", Hotkey('A')),
		NF(f_Right, u8"右", "Visuals::FreeCamera", Hotkey('D')),
		NF(f_Up, u8"上", "Visuals::FreeCamera", Hotkey(VK_SPACE)),
		NF(f_Down, u8"下", "Visuals::FreeCamera", Hotkey(VK_LCONTROL)),
		NF(f_LeftRoll, u8"向左滚动", "Visuals::FreeCamera", Hotkey('Z')),
		NF(f_RightRoll, u8"向右滚动", "Visuals::FreeCamera", Hotkey('X')),
		NF(f_ResetRoll, u8"Reset Roll", "Visuals::FreeCamera", Hotkey('C')),
		NF(f_IncFOV, u8"增加视野", "Visuals::FreeCamera", Hotkey('3')),
		NF(f_DecFOV, u8"降低视场", "Visuals::FreeCamera", Hotkey('1'))
	{
	...
	{
		static const FeatureGUIInfo info{ u8"自由视角", u8"图像渲染", true };
		return info;
	}
	...
	void FreeCamera::DrawMain()
	{
		...
		ConfigWidget(u8"冻结角色动画", f_FreezeAnimation, u8"冻结活动角色的动画.");
		if (f_Enabled)
		{
			ConfigWidget(u8"切换损伤覆盖", f_DamageOverlay, u8"移除损坏输出覆盖层");
			ConfigWidget(u8"切换敌人的HP覆盖", f_HpOverlay, u8"移除敌人的HP覆盖");
		}

		if (ImGui::BeginTable(u8"可绘制自由视角", 1, ImGuiTableFlags_NoBordersInBody))
		{
			ImGui::TableNextRow();
			...
			ImGui::BeginGroupPanel(u8"设置");
			{
				...
				ConfigWidget(u8"视野", f_FOV, 0.1f, 0.01f, 200.0f, u8"改变垂直视场。水平FoV取决于视口的纵横比");
				if (ImGui::Button(u8"将视场转换为35mm FF焦距"))
				...
				ImGui::Spacing();
				ConfigWidget(u8"运动平滑", f_MovSmoothing, 0.01f, 0.001f, 1.0f, "Lower = Smoother");
				ConfigWidget(u8"外观平滑", f_LookSmoothing, 0.01f, 0.001f, 1.0f, "Lower = Smoother");
				ConfigWidget(u8"滚动平滑", f_RollSmoothing, 0.01f, 0.001f, 1.0f, "Lower = Smoother");
				ConfigWidget(u8"视场平滑", f_FovSmoothing, 0.01f, 0.001f, 1.0f, "Lower = Smoother");
			}
			ImGui::EndGroupPanel();

			ImGui::BeginGroupPanel(u8"快捷键");
			{
				...
				ConfigWidget(u8"重置滚动", f_ResetRoll, true);
				ConfigWidget(u8"增加视野", f_IncFOV, true);
				ConfigWidget(u8"降低视野", f_DecFOV, true);
			}
			ImGui::EndGroupPanel();
			ImGui::EndTable();
		}

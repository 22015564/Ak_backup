...
namespace cheat::feature
{
    ...
    ProfileChanger::ProfileChanger() : Feature(),
        NF(f_Enabled, u8"自定义资料", u8"图像渲染::自定义资料", false),
        NF(f_UID, "UID", u8"图像渲染::自定义资料", false),
        NF(f_UIDWaterMarkPrefix, "UIDWaterMarkPrefix", u8"图像渲染::自定义资料", false),
        NF(f_UIDsize, u8"UID 大小", u8"图像渲染::自定义资料", 14),
        NF(f_UIDpos_x, u8"UID X坐标", u8"图像渲染::自定义资料", static_cast<float>(app::Screen_get_width(nullptr) * 0.96875)),
        NF(f_UIDpos_y, u8"UID Y坐标", u8"图像渲染::自定义资料", 0),
        NF(f_NickName, u8"昵称", u8"图像渲染::自定义资料", false),
        NF(f_Level, u8"等级", u8"图像渲染::自定义资料", false),
        NF(f_Exp, u8"经验", u8"图像渲染::自定义资料", false),
        NF(f_CurExp, u8"当前经验", u8"图像渲染::自定义资料", 1),
        NF(f_MaxExp, u8"最大经验", u8"图像渲染::自定义资料", 1),
        NF(f_ExpBar, u8"经验条", u8"图像渲染::自定义资料", false),
        NF(f_ExpBarValue, "ExpBarValue", u8"图像渲染::自定义资料", 20.0f),
        NF(f_WorldLevel, u8"世界等级", u8"图像渲染::自定义资料", false),
        NF(f_Avatar, u8"头像", u8"图像渲染::自定义资料", false),
        NF(f_Card, u8"名片", u8"图像渲染::自定义资料", false),
    ...
    {
        static const FeatureGUIInfo info{ u8"自定义资料", u8"图像渲染", true };
        return info;
    }

    void ProfileChanger::DrawMain()
    {
        ConfigWidget(f_Enabled, u8"自定义资料.");
        ConfigWidget(f_UID, u8"修改uid.");
        ConfigWidget(u8"在水印上附加 \"UID:\" 前缀", f_UIDWaterMarkPrefix);
        ConfigWidget(u8"UID 大小", f_UIDsize, 0.1, 1, 500.0, u8"设置 UID 大小");
        ConfigWidget(u8"UID X坐标", f_UIDpos_x, 1.f, 1.f, static_cast<float>(app::Screen_get_width(nullptr)), u8"设置 UID X坐标");
        ConfigWidget(u8"UID y坐标", f_UIDpos_y, 1.f, 0, static_cast<float>(app::Screen_get_height(nullptr)), u8"设置 UID y坐标");
        ConfigWidget(f_NickName, u8"更改昵称.");
        ConfigWidget(f_Level, u8"更改等级.");
        ConfigWidget(f_Exp, u8"更改经验值.");
        if (f_Exp) {
            ConfigWidget(u8"当前经验", f_CurExp, 1, 2, 100000, u8"更改显示的当前经验值.");
            ConfigWidget(u8"最大经验", f_MaxExp, 1, 2, 100000, u8"更改显示的升级所需经验值.");
            ConfigWidget(f_ExpBar, "Changes the ExpBar visually.");
            if (f_ExpBar)
                ConfigWidget(u8"经验条数值", f_ExpBarValue, 1, 2, 100, u8"修改经验条数值.");
        }
        ConfigWidget(f_WorldLevel, u8"更改世界等级.");
        ConfigWidget(f_Avatar, u8"本地修改头像.\n" \
            u8"注意图片大小必须是: 256x256.\n" \
            u8"路径像这样填: C:\\Avatars.png");

        ConfigWidget(f_Card, u8"本地修改名片背景.\n" \
            u8"注意名片背景大小必须是: 840x400.\n" \
            u8"路径像这样填: C:\\Avatars.png");
    }

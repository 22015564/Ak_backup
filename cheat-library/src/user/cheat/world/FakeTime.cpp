...
namespace cheat::feature
{
	...
	FakeTime::FakeTime() : Feature(),
		NF(f_Enabled, u8"伪造时间", u8"开启", false),
		NF(f_TimeHour, u8"伪造时间", "TimeHour", 12),
		NF(f_TimeMinute, u8"伪造时间", "TimeMinute", 0)
	{
	...
	{
		static const FeatureGUIInfo info{ u8"伪造时间", u8"世界", true };
		return info;
	}
	void FakeTime::DrawMain()
	{
		ConfigWidget(u8"开启", f_Enabled, u8"保持游戏时间不变");
	...
	{
		ImGui::Text(u8"伪造时间|%d:%d", f_TimeHour.value(), f_TimeMinute.value());
	}

...
namespace cheat::feature
{
    ...
    DialogSkip::DialogSkip() : Feature(),
        ...
        NF(f_AutoSelectDialog, u8"自动选择回答", u8"自动对话", true),
        NF(f_ExcludeImportant, u8"不包括凯瑟琳/Tubby/Wagner", u8"自动对话", true),
        NF(f_FastDialog, u8"快速对话", u8"自动对话", false),
        NF(f_CutsceneUSM, u8"跳过动画", u8"自动对话", false),
        NF(f_TimeSpeedup, u8"时间速度", u8"自动对话", 5.0f)
    {
    ...
    void DialogSkip::DrawMain()
    {
        ...
        {
            ImGui::Indent();
            ConfigWidget(u8"不包括凯瑟琳/Tubby/Wagner", f_ExcludeImportant, u8"不包括凯瑟琳/Tubby/Wagner 从自动选择.");
            ImGui::Unindent();
        }
        ConfigWidget(u8"快速对话", f_FastDialog, u8"加速时间");
        if (f_FastDialog)
        {
            ConfigWidget(f_TimeSpeedup, 0.1f, 2.0f, 50.0f, u8"时间加速倍增器 \n较高的值将导致与服务器的同步问题 \n不建议用于延迟的Internet连接.");
        }

...
namespace cheat::feature
{
	AutoTreeFarm::AutoTreeFarm() : Feature(),
		NF(m_Enabled, u8"自动伐树", u8"自动伐树", false),
		NF(m_AttackDelay, u8"攻击延迟", u8"自动伐树", 150),
		NF(m_RepeatDelay, u8"重复延迟", u8"自动伐树", 500),
		NF(m_AttackPerTree, u8"每树攻击次数", u8"自动伐树", 5),
		NF(m_Range, u8"范围", u8"自动伐树", 15.0f)
	{
	...
	{
		static const FeatureGUIInfo info{ u8"自动伐树", u8"世界", true };
		return info;
	}

	void AutoTreeFarm::DrawMain()
	{
		ImGui::TextColored(ImColor(255, 165, 0, 255), u8"注意. 该功能未经行全面测试.\n"
			u8"不建议用于主帐户或高数值使用.");

		ConfigWidget(u8"开启", m_Enabled, u8"自动攻击范围内的树.");
		ConfigWidget(u8"攻击延迟 (毫秒)", m_AttackDelay, 1, 0, 1000, u8"在攻击下一棵树之前的延迟 (毫秒).");
		ConfigWidget(u8"重复延迟 (毫秒)", m_RepeatDelay, 1, 500, 1000, u8"攻击同一棵树的间隔 (毫秒).\n数值 <500毫秒 将不工作.");

		ConfigWidget(u8"每树攻击次数", m_AttackPerTree, 1, 0, 100, u8"每一棵树的攻击次数.\n" \
			u8"建议设置为10或更低，以避免攻击空树.\n" \
			u8"设置为0表示无限制攻击 (即使已经不掉落木材，风险极高).\n" \
			u8"注意: 游戏重启后，记录已经攻击的数据被重置."
		);

		ConfigWidget(u8"范围 (米)", m_Range, 0.1f, 1.0f, 15.0f);
		ImGui::TextColored(ImColor(255, 165, 0, 255), u8"为了安全起见，范围被软限制在~15m.");
	}
